	...
	async execute(message, args, fb) {
		...
		async function prepareEmbed(result, scoreLink, liste, firstCut, secondCut) {
			...
			listeDisplay.forEach((element) => {
				...
				let dateSubmitted = moment(score.raw_date, 'YYYY-MM-DD HH:mm:SS').fromNow();
				...
				let enabledMods = score.mods.toString();
        enabledMods = modFormat.execute(enabledMods);

				embedConstructor.addField(
					`**${element}. ${mapArtist} - ${mapTitle} [${mapDiff}] +${enabledMods}**`,
		...
		message.channel.send(embed).then((embedEdit) => {
			const filter = (reaction, user) => {
				return [ '🔼', '🔽' ].includes(reaction.emoji.name) && user.id !== embedEdit.author.id;
			};
			...
			async function reactionWait(filter, firstCut, secondCut, embedEdit, liste) {
				embedEdit

...
module.exports = {
	...
	async execute(message, args, fb) {
		...
		if (!args.length) {
			...
			if (!userID) return message.reply("vous devez link votre compte osu, plus d'infos avec `;help osulink`");
		} else userID = args.join('_'); // else define the player as the first argument

		let recentMap = await fb.collection('guilds').doc(message.guild.id).get().then((q) => {
			return q.data().recentMap;
		});

		recentMap = parseFloat(recentMap);
		...
		const scoreData = osuApi // parsing scores of the user on the corresponding beatmap
			...
			.then((scores) => {
				if (!scores[0])
					...
					return message.reply("cet utilisateur n'a aucun score sur cette map"), msg.delete();
				fetch(`https://osu.ppy.sh/api/get_scores?k=${apiKey.osuToken}&b=${recentMap}&u=${userID}`)
					...
					.then(async (body) => {
						...
						async function sendEmbed(message, scores, scoresToCheck, submitCount, userID) {
							...
							let mapLength = lengthFormat.execute(scores[0]._beatmap.length['total']);
							let mapBPM = scores[0]._beatmap.bpm;

							let userRank = await osuApi.apiCall('/get_user', { u: userID }).then((uData) => {
								return uData[0].pp_rank;
							});

							userRank = numberFormat.execute(userRank);

							let embed = new MessageEmbed() // embed constructor
								.setColor(embedColor)
								.setAuthor(
									`Now comparing to ${scores[0].user.name} (#${userRank})`,
									`https://a.ppy.sh/${scores[0].user.id}?1569169881.png`,
									`https://osu.ppy.sh/users/${scores[0].user.id}`
								)
								.setImage(
									`https://assets.ppy.sh/beatmaps/${scores[0]._beatmap
								...
								.setTitle(
									`${scores[0]._beatmap.artist} - ${scores[0]._beatmap.title} [${scores[0]._beatmap
										.version}]`
								)
								.setURL(
									`https://osu.ppy.sh/beatmapsets/${scores[0]._beatmap.beatmapSetId}#osu/${scores[0]
										._beatmap.id}`
								);
							...
							if (scoresToCheck[1] - 1 !== submitCount) {
								...
								let accuracy1 = (scores[scoresToCheck[0] - 1].accuracy * 100).toFixed(2);
								let accuracy2 = (scores[scoresToCheck[1] - 1].accuracy * 100).toFixed(2);
								...
								if (scores[scoresToCheck[0] - 1].maxCombo >= scores[0]._beatmap.maxCombo - 10) {
									FC1 = true;
								}
								let FC2 = false; // detect if the second score is an fc
								if (scores[scoresToCheck[1] - 1].maxCombo >= scores[0]._beatmap.maxCombo - 10) {
									FC2 = true;
								}
								...
								let timeAgo2 = moment(raw_date2, 'YYYY-MM-DD HH:mm:SS').fromNow();
              ...
              **${rankingIcon1} 🠶 ${pp1}pp ${embedPPIF}
              🠶 ${(scores[scoresToCheck[0] - 1].accuracy * 100).toFixed(2)}% | ${scores[scoresToCheck[0] - 1].counts[
											'100'
										]} / ${scores[scoresToCheck[0] - 1].counts['50']} / ${scores[
											scoresToCheck[0] - 1
										].counts.miss}m
              🠶 ${totalScore1} | ${scores[scoresToCheck[0] - 1].maxCombo} / ${scores[0]._beatmap.maxCombo}
              ...
              **${rankingIcon2} 🠶 ${pp2}pp ${embedPPIF2}
              🠶 ${(scores[scoresToCheck[1] - 1].accuracy * 100).toFixed(2)}% | ${scores[scoresToCheck[1] - 1].counts[
											'100'
										]} / ${scores[scoresToCheck[1] - 1].counts['50']} / ${scores[
											scoresToCheck[1] - 1
										].counts.miss}m
              🠶 ${totalScore2} | ${scores[scoresToCheck[1] - 1].maxCombo} / ${scores[0]._beatmap.maxCombo}
              ...
              **${rankingIcon1} 🠶 ${pp1}pp ${embedPPIF}
              🠶 ${(scores[scoresToCheck[0] - 1].accuracy * 100).toFixed(2)}% | ${scores[scoresToCheck[0] - 1].counts[
										'100'
									]} / ${scores[scoresToCheck[0] - 1].counts['50']} / ${scores[scoresToCheck[0] - 1]
										.counts.miss}m
              🠶 ${totalScore1} | ${scores[scoresToCheck[0] - 1].maxCombo} / ${scores[0]._beatmap.maxCombo}
                  🠶 [Score set ${timeAgo1}](https://osu.ppy.sh/scores/osu/${scoreID1})**`,
									inline: true
						...
						let embed1 = await sendEmbed(message, scores, scoresToCheck, submitCount, userID);
						msg.delete();
							...
							embedEdit
								...
								.then(async (collected) => {
									...
									if (reaction.emoji.name === '2️⃣') {
										...
										let embed2 = await sendEmbed(message, scores, scoresToCheck, submitCount, userID);
